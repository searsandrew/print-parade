...
test('the selected user authorizes and prepares a zpl print job', function () {
    ...
    $prepared = app(PrintJobExecutor::class)->prepare($job, $selectedUser, '4826', 203);
...
test('an incorrect pin does not start the print job', function () {
    ...
    expect(fn () => app(PrintJobExecutor::class)->prepare($job, $user, '1111', 203))
...
test('a selected user without a configured pin cannot authorize printing', function () {
    ...
    expect(fn () => app(PrintJobExecutor::class)->prepare($job, $user, '4826', 203))
 ...
 */
function executablePrintJob(array $overrides = []): PrintJob
{
    ...
    return PrintJob::factory()->for($version)->create([
